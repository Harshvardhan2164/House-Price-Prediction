...
df = pd.read_csv('house.csv') # Read the housing data from CSV file
...
df.dropna(inplace = True) # Drops any rows with missing valules from the Dataframe

print(cl(df.isnull().sum(), attrs = ['bold'])) # Print the number of missing values for each column

print(df.describe()) # Print sumaary statistics(mean, count, standard deviation, min, max, quartiles) of the dataset
...
''' Scatter plot is also useed to observe linear relations between two variables in dataset.
    ...
    calles with 'SalePrice' as the argument to create the scatter plots.
'''
...
models = ['Ordinary Least Squares', 'Ridge', 'Lasso', 'Bayesian Ridge', 'Elastic Net']
mse = [np.mean((ols_pred - y_test)**2), np.mean((ridge_pred - y_test)**2),
       np.mean((lasso_pred - y_test)**2), np.mean((bayesian_pred - y_test)**2)]

print(cl(pd.DataFrame({'Model': models, 'Mean Squared Error': mse}).to_string(index=False), attrs=['bold']))

plt.bar(models, mse, color=['orange', 'purple', 'green', 'blue', 'yellow'])  # Create a bar plot of the MSE values
plt.title('Model Comparison', fontsize=16)  # Set the plot title
plt.xlabel('Model', fontsize=14)  # Set the x-axis label
plt.ylabel('Mean Squared Error', fontsize=14)  # Set the y-axis label
plt.xticks(rotation=45, fontsize=12)  # Rotate and set the x-axis tick labels font size
plt.yticks(fontsize=12)  # Set the y-axis tick labels font size
plt.savefig('model_comparison.png')  # Save the bar plot as an image
...
# 1. OLS
# Plotting the sale prices for training data

training_data_prediction = ols.predict(X_train)
...
training_data_prediction = bayesian.predict(X_train)
plt.scatter(y_train, training_data_prediction)
plt.xlabel("Actual Prices")
plt.ylabel("Predicted Prices")
plt.title("Actual Price vs Predicted Price")
...
# EVALUATION
# Comparing the 5 models to find the best model for the house price prediction
